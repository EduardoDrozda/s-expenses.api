...
@Injectable()
export class UpdateCostCenterUseCase implements IBaseUseCase<UpdateCostCenterRequestDto, GetCostCenterResponseDto>{
  constructor(
    @Inject(COST_CENTER_REPOSITORY) private readonly costCenterRepository: ICostCenterRepository,
    private readonly logger: LoggerService
  ) {}

  async execute(data: UpdateCostCenterRequestDto): Promise<GetCostCenterResponseDto> {
    ...
    const updatedCostCenter = await this.costCenterRepository.update(data);
